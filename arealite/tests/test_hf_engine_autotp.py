...
def dist_env():
    ...
    if not torch.distributed.is_initialized():
        ...
        print(f"Rank {local_rank}: Distributed initialized, WORLD_SIZE={torch.distributed.get_world_size()}", flush=True)
    yield
    if torch.distributed.is_initialized():
        torch.distributed.destroy_process_group()
        print(f"Rank {local_rank}: Distributed destroyed", flush=True)
